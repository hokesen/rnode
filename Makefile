...
ARDUINO_CLI = arduino-cli --config-file arduino-cli.yaml
ARDUINO_DATA_DIR = ./.arduino15
ESP32_CORE_PATH = $(ARDUINO_DATA_DIR)/packages/esp32/hardware/esp32/$(ARDUINO_ESP_CORE_VER)
ESP32_BOOT_APP0 = $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin
SKETCH_NAME = RNode_Firmware_CE
SKETCH_ROOT = ./.sketch
SKETCH_DIR = $(SKETCH_ROOT)/$(SKETCH_NAME)
SKETCH_BUILD_DIR = $(SKETCH_DIR)/build
...
COMMON_BUILD_FLAGS =  $(VFLAG) -e --build-property "build.partitions=no_ota" --build-property "upload.maximum_size=2097152" $(SKETCH_DIR)
...
SKETCH_PREP_TARGETS = $(shell awk -F: '/^(firmware|upload|release)-/ {print $$1}' Makefile)

all: release

clean:
	-rm -rf ./build $(SKETCH_ROOT)
	-rm -f ./Release/rnode_firmware*

.PHONY: prepare-sketch
...
prepare-sketch:
	...
	@rsync -a --delete \
		--exclude '.git/' \
		--exclude '.arduino/' \
		--exclude '.arduino-cache/' \
		--exclude '.arduino15/' \
		--exclude '.sketch/' \
		--exclude 'build' \
		--exclude 'Console/build/' \
		--exclude 'Documentation/vendor/' \
		--exclude 'Release/*.hex' \
	...
	@rm -rf ./build
	@ln -s .sketch/$(SKETCH_NAME)/build ./build

$(SKETCH_PREP_TARGETS): prepare-sketch
...
prep-esp32:
	$(ARDUINO_CLI) core install esp32:esp32@$(ARDUINO_ESP_CORE_VER)
	$(ARDUINO_CLI) lib install "Adafruit SSD1306"
	$(ARDUINO_CLI) lib install "Adafruit SH110X"
	$(ARDUINO_CLI) lib install "Adafruit ST7735 and ST7789 Library"
...
prep-nrf:
	...
	$(ARDUINO_CLI) core install rakwireless:nrf52
	$(ARDUINO_CLI) core install Heltec_nRF52:Heltec_nRF52
	$(ARDUINO_CLI) lib install "Crypto"
	...
	$(ARDUINO_CLI) lib install --git-url https://github.com/liamcottle/esp8266-oled-ssd1306#e16cee124fe26490cb14880c679321ad8ac89c95
	pip install pyserial rns --upgrade --user --break-system-packages # This looks scary, but it's actually just telling pip to install packages as a user instead of trying to install them systemwide, which bypasses the "externally managed environment" error.
...
check_bt_buffers:
	@./esp32_btbufs.py $(ESP32_CORE_PATH)/libraries/BluetoothSerial/src/BluetoothSerial.cpp

firmware-tbeam: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:t-beam $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x33\" \"-DBOARD_VARIANT=0xE4\""

firmware-tbeam_sx1262: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:t-beam $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x33\" \"-DBOARD_VARIANT=0xE8\""

firmware-techo:
	$(ARDUINO_CLI) compile --fqbn adafruit:nrf52:pca10056 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x44\""

firmware-t3s3:
	$(ARDUINO_CLI) compile --fqbn "esp32:esp32:esp32s3:CDCOnBoot=cdc" $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x42\" \"-DBOARD_VARIANT=0xAB\""

firmware-t3s3_sx126x:
	$(ARDUINO_CLI) compile --fqbn "esp32:esp32:esp32s3:CDCOnBoot=cdc" $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x42\" \"-DBOARD_VARIANT=0xA1\""

firmware-t3s3_sx1280_pa:
	$(ARDUINO_CLI) compile --fqbn "esp32:esp32:esp32s3:CDCOnBoot=cdc" $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x42\" \"-DBOARD_VARIANT=0xAC\""

firmware-e22_esp32:
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:esp32 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x45\" \"-DEXTERNAL_LEDS=true\""

firmware-lora32_v10: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:ttgo-lora32 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x39\""

firmware-lora32_v10_extled: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:ttgo-lora32 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x39\" \"-DEXTERNAL_LEDS=true\""

firmware-lora32_v20: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:ttgo-lora32 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x36\" \"-DEXTERNAL_LEDS=true\""

firmware-lora32_v21: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:ttgo-lora32 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x37\""

firmware-lora32_v21_extled: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:ttgo-lora32 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x37\" \"-DEXTERNAL_LEDS=true\""

firmware-lora32_v21_tcxo: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:ttgo-lora32 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x37\" \"-DENABLE_TCXO=true\""

firmware-heltec32_v2: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:heltec_wifi_lora_32_V2 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x38\""

firmware-heltec32_v2_extled: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:heltec_wifi_lora_32_V2 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x38\" \"-DEXTERNAL_LEDS=true\""

firmware-heltec32_v3:
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:heltec_wifi_lora_32_V3 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x3A\""

firmware-heltec32_v4:
	$(ARDUINO_CLI) compile --fqbn "esp32:esp32:esp32s3:CDCOnBoot=cdc" $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x43\""

firmware-heltec_w_paper:
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:heltec_wifi_lora_32_V3 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x3E\""

firmware-xiao_s3:
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:XIAO_ESP32S3 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x3E\""

firmware-rnode_ng_20: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:ttgo-lora32 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x40\""

firmware-rnode_ng_21: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:ttgo-lora32 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x41\""

firmware-featheresp32: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:featheresp32 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x34\""

firmware-genericesp32: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:esp32 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x35\""

firmware-rak4631:
	$(ARDUINO_CLI) compile --fqbn rakwireless:nrf52:WisCoreRAK4631Board $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x51\" \"-DBOARD_VARIANT=0x12\""

firmware-rak4631_sx1280:
	$(ARDUINO_CLI) compile --fqbn rakwireless:nrf52:WisCoreRAK4631Board $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x51\" \"-DBOARD_VARIANT=0x14\""

firmware-opencom-xl:
	$(ARDUINO_CLI) compile --fqbn rakwireless:nrf52:WisCoreRAK4631Board $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x52\" \"-DBOARD_VARIANT=0x21\""

firmware-heltec_t114:
	$(ARDUINO_CLI) compile --log --fqbn Heltec_nRF52:Heltec_nRF52:HT-n5262 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x3C\""

firmware-heltec_t114_gps:
	$(ARDUINO_CLI) compile --log --fqbn Heltec_nRF52:Heltec_nRF52:HT-n5262 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x3C\" \"-DBOARD_VARIANT=0xCB\""

upload-tbeam:
	$(ARDUINO_CLI) upload -p $(or $(port), /dev/ttyACM0) --fqbn esp32:esp32:t-beam $(SKETCH_DIR)
	@sleep 1
...
upload-tbeam_sx1262:
	$(ARDUINO_CLI) upload -p /dev/ttyACM0 --fqbn esp32:esp32:t-beam $(SKETCH_DIR)
	@sleep 1
...
upload-lora32_v10:
	$(ARDUINO_CLI) upload -p $(or $(port), /dev/ttyUSB0) --fqbn esp32:esp32:ttgo-lora32 $(SKETCH_DIR)
	@sleep 1
...
upload-lora32_v21:
	$(ARDUINO_CLI) upload -p $(or $(port), /dev/ttyACM0) --fqbn esp32:esp32:ttgo-lora32 $(SKETCH_DIR)
	@sleep 1
...
upload-heltec32_v2:
	$(ARDUINO_CLI) upload -p $(or $(port), /dev/ttyUSB0) --fqbn esp32:esp32:heltec_wifi_lora_32_V2 $(SKETCH_DIR)
	@sleep 1
...
upload-heltec_w_paper upload-heltec32_v3:
	$(ARDUINO_CLI) upload -p $(or $(port), /dev/ttyUSB0) --fqbn esp32:esp32:heltec_wifi_lora_32_V3 $(SKETCH_DIR)
	@sleep 1
...
upload-heltec32_v4:
	$(ARDUINO_CLI) upload -p $(or $(port), /dev/ttyACM0) --fqbn "esp32:esp32:esp32s3:CDCOnBoot=cdc" $(SKETCH_DIR)
	@sleep 1
...
upload-xiao_s3:
	$(ARDUINO_CLI) upload -p $(or $(port), /dev/ttyACM0) --fqbn esp32:esp32:XIAO_ESP32S3 $(SKETCH_DIR)
	@sleep 1
...
upload-tdeck:
	$(ARDUINO_CLI) upload -p $(or $(port), /dev/ttyACM0) --fqbn esp32:esp32:esp32s3 $(SKETCH_DIR)
	@sleep 1
	rnodeconf $(or $(port), /dev/ttyACM0) --firmware-hash $$(./partition_hashes ./build/esp32.esp32.esp32s3/RNode_Firmware_CE.ino.bin)
	@sleep 3
...
upload-featheresp32:
	$(ARDUINO_CLI) upload -p $(or $(port), /dev/ttyUSB0) --fqbn esp32:esp32:featheresp32 $(SKETCH_DIR)
	@sleep 1
...
upload-opencom-xl upload-rak4631:
	$(ARDUINO_CLI) upload -p $(or $(port), /dev/ttyACM0) --fqbn rakwireless:nrf52:WisCoreRAK4631Board $(SKETCH_DIR)
...
upload-e22_esp32:
	$(ARDUINO_CLI) upload -p $(or $(port), /dev/ttyUSB0) --fqbn esp32:esp32:esp32 $(SKETCH_DIR)
	@sleep 1
...
upload-heltec_t114:
	$(ARDUINO_CLI) upload -p /dev/ttyACM0 --fqbn Heltec_nRF52:Heltec_nRF52:HT-n5262 $(SKETCH_DIR)
	@sleep 1
	rnodeconf /dev/ttyACM0 --firmware-hash $$(./partition_hashes from_device /dev/ttyACM0)

upload-techo:
	$(ARDUINO_CLI) upload -p /dev/ttyACM0 --fqbn adafruit:nrf52:pca10056 $(SKETCH_DIR)
	@sleep 6
	rnodeconf /dev/ttyACM0 --firmware-hash $$(./partition_hashes from_device /dev/ttyACM0)
...
release-tbeam: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:t-beam $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x33\" \"-DBOARD_VARIANT=0xE4\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_tbeam.boot_app0
...
release-tbeam_sx1262: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:t-beam $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x33\" \"-DBOARD_VARIANT=0xE8\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_tbeam_sx1262.boot_app0
...
release-lora32_v10: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:ttgo-lora32 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x39\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_lora32v10.boot_app0
...
release-lora32_v20: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:ttgo-lora32 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x36\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_lora32v20.boot_app0
...
release-lora32_v21: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:ttgo-lora32 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x37\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_lora32v21.boot_app0
...
release-lora32_v21_tcxo: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:ttgo-lora32 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x37\" \"-DENABLE_TCXO=true\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_lora32v21_tcxo.boot_app0
...
release-heltec32_v2: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:heltec_wifi_lora_32_V2 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x38\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_heltec32v2.boot_app0
...
release-heltec32_v3:
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:heltec_wifi_lora_32_V3 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x3A\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_heltec32v3.boot_app0
...
release-heltec32_v4:
	$(ARDUINO_CLI) compile --fqbn "esp32:esp32:esp32s3:CDCOnBoot=cdc" $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x43\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_heltec32v4.boot_app0
	cp build/esp32.esp32.esp32s3/RNode_Firmware_CE.ino.bin build/rnode_firmware_heltec32v4.bin
	cp build/esp32.esp32.esp32s3/RNode_Firmware_CE.ino.bootloader.bin build/rnode_firmware_heltec32v4.bootloader
	cp build/esp32.esp32.esp32s3/RNode_Firmware_CE.ino.partitions.bin build/rnode_firmware_heltec32v4.partitions
	zip --junk-paths ./Release/rnode_firmware_heltec32v4.zip ./Release/esptool/esptool.py ./Release/console_image.bin build/rnode_firmware_heltec32v4.boot_app0 build/rnode_firmware_heltec32v4.bin build/rnode_firmware_heltec32v4.bootloader build/rnode_firmware_heltec32v4.partitions
	rm -r build

release-heltec_w_paper:
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:heltec_wifi_lora_32_V3 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x3E\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_heltecwpaper.boot_app0
...
release-xiao_s3:
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:XIAO_ESP32S3 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x3E\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_xiao_esp32s3.boot_app0
...
release-rnode_ng_20: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:ttgo-lora32 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x40\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_ng20.boot_app0
...
release-rnode_ng_21: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:ttgo-lora32 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x41\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_ng21.boot_app0
...
release-t3s3:
	$(ARDUINO_CLI) compile --fqbn "esp32:esp32:esp32s3:CDCOnBoot=cdc" $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x42\" \"-DBOARD_VARIANT=0xA1\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_t3s3_sx126x.boot_app0
...
release-t3s3_sx127x:
	$(ARDUINO_CLI) compile --fqbn "esp32:esp32:esp32s3:CDCOnBoot=cdc" $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x42\" \"-DBOARD_VARIANT=0xA5\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_t3s3_sx127x.boot_app0
...
release-t3s3_sx1280_pa:
	$(ARDUINO_CLI) compile --fqbn "esp32:esp32:esp32s3:CDCOnBoot=cdc" $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x42\" \"-DBOARD_VARIANT=0xAC\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_t3s3_sx1280_pa.boot_app0
...
release-e22_esp32:
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:esp32 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x45\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_esp32_e22.boot_app0
...
release-tdeck:
	$(ARDUINO_CLI) compile --fqbn "esp32:esp32:esp32s3:CDCOnBoot=cdc" $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x3B\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_tdeck.boot_app0
...
release-tbeam_supreme:
	$(ARDUINO_CLI) compile --fqbn "esp32:esp32:esp32s3:CDCOnBoot=cdc" $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x3D\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_tbeam_supreme.boot_app0
...
release-featheresp32: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:featheresp32 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x34\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_featheresp32.boot_app0
...
release-genericesp32: check_bt_buffers
	$(ARDUINO_CLI) compile --fqbn esp32:esp32:esp32 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x35\""
	cp $(ESP32_CORE_PATH)/tools/partitions/boot_app0.bin build/rnode_firmware_esp32_generic.boot_app0
...
release-heltec_t114:
	$(ARDUINO_CLI) compile --fqbn Heltec_nRF52:Heltec_nRF52:HT-n5262 $(COMMON_BUILD_FLAGS) --build-property "compiler.cpp.extra_flags=\"-DBOARD_MODEL=0x3C\""
